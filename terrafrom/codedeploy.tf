...
resource "aws_codedeploy_deployment_group" "demo" {
 ...
 load_balancer_info {
    target_group_pair_info {
      prod_traffic_route {
        listener_arns = [aws_alb_listener.demo.arn]
      }
  test_traffic_route {
        listener_arns = [aws_alb_listener.demo-green.arn]
      }

     target_group {
        name = aws_alb_target_group.demo-blue.name
      }

      target_group {
        name = aws_alb_target_group.demo-green.name
      }
    }
  }
